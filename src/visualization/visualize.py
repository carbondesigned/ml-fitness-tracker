...
def plot(subset_count, label):
    subset = df[df["label"] == label]
    fig, ax = plt.subplots()
    plt.plot(subset[:subset_count]["acc_y"].reset_index(
        drop=True), label=label)
    plt.legend()
    plt.show()
...
combined_plot_df = (
    df.query(f"label == '{label}'").query(
        f'participant == "{participant}"').reset_index().reset_index(drop=True)
)
...
ax[0].legend(loc="upper left", bbox_to_anchor=(
    0.5, 1.15), ncol=3, fancybox=True, shadow=True)
ax[1].legend(loc="upper left", bbox_to_anchor=(
    0.5, 1.15), ncol=3, fancybox=True, shadow=True)
